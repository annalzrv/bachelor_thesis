...
\setStudent{Lazareva Anna Alexandrovna}
...
Physics-informed neural networks (PINNs) approximate PDE solutions by minimising a weighted combination of residual, boundary, initial, and data losses. Performance is dominated by two choices fixed before training: the loss weights and the physical parameters of the PDE. A poor weighting collapses training onto one constraint; a different parameter value requires retraining from scratch. Existing remedies select or adapt a single configuration. We take a different view: instead of tuning one vector, we condition the network on it and explore the entire family during training.

This thesis develops the \emph{loss-conditional PINN} (\LCPINN{}), a single network $u_\theta(x, \param)$ in which the conditioning vector $\param$ --- either the loss-weight simplex or a physical PDE parameter --- is an input. Training minimises an expected loss over a prior on $\param$, so one network covers a continuous family of problems with no solver-generated paired data. One architecture and one algorithm cover both regimes; FiLM modulation~\cite{perez2018film} with an L-BFGS refinement on a fixed quadrature support handles the parametric-coefficient case, and concatenation conditioning~\cite{dosovitskiy2020} handles the loss-weight case.

Empirically, a single \LCPINN{} improves over per-parameter retrained baselines by $3$--$6\times$ on Helmholtz at $d = 1, 2, 3$ and on Schr\"{o}dinger; in the loss-weight regime it matches the strongest adaptive-weight baselines on viscous Burgers and on viscous-regularised Buckley--Leverett displacement --- the porous-media application that motivates the work. Two theoretical results frame the empirical study: a $\param$-invariance result that LC minimisers recover residual minimisers $p_\param$-almost everywhere, and a local loss-decay advantage that the conditional parameterisation enlarges the descent set rather than shifting the optimum.

A short interpretability section applies a functional ANOVA decomposition to the trained two-dimensional Helmholtz \LCPINN{} in the joint space of the spatial coordinates and the wavenumber. The decomposition isolates a dominant three-way interaction among the two spatial coordinates and the wavenumber, from which we derive an a-priori lower bound on the relative-$L^2$ error attainable by any architecturally simpler solver --- additive, or pairwise-additive --- on the same parametric family. The bound is verified against the analytic projection on the manufactured reference solution.
...
Физически информированные нейронные сети (PINN) приближают решения УЧП минимизацией взвешенной комбинации остаточных, граничных, начальных и дата-потерь. Эффективность определяется двумя выборами, фиксируемыми до обучения: весами потерь и значениями физических параметров уравнения. Плохое взвешивание приводит к вырожденному решению; иное значение параметра требует переобучения с нуля. Существующие методы подбирают одну конфигурацию. Мы предлагаем другой взгляд: вместо подгонки одного вектора подаём его в сеть и исследуем всё семейство в ходе обучения.

В работе развивается \emph{loss-conditional PINN} (\LCPINN{}) --- единая сеть $u_\theta(x, \param)$, в которой обусловливающий вектор $\param$ (симплекс весов потерь либо физический параметр УЧП) является входом. Минимизируется ожидаемая функция потерь по априорному распределению $p_\param$, так что одна сеть охватывает непрерывное семейство задач без сгенерированных солвером парных данных. Один и тот же алгоритм работает в обоих режимах: FiLM-модуляция~\cite{perez2018film} с L-BFGS-доводкой на фиксированной квадратуре в параметрическом режиме, конкатенация~\cite{dosovitskiy2020} --- в режиме весов потерь.

Эмпирически один \LCPINN{} улучшает per-parameter переобученные бейзлайны в $3$--$6\times$ на Гельмгольце в $d = 1, 2, 3$ и на Шрёдингере; в режиме весов потерь --- сравним с сильнейшими адаптивно-взвешенными бейзлайнами на вязком Бюргерсе и на вязко-регуляризованном Бакли--Леверетте (приложение к пористым средам, мотивирующее работу). Два теоретических результата обрамляют эмпирику: $\param$-инвариантность минимумов и локальное преимущество скорости убывания лосса у условной параметризации.

Краткий раздел по интерпретируемости применяет функциональное ANOVA-разложение к обученному двумерному Гельмгольц-\LCPINN{} в совместном пространстве пространственных координат и волнового числа. Разложение выявляет доминирующее тройное взаимодействие двух пространственных координат и волнового числа, из которого выводится априорная нижняя граница относительной $L^2$-ошибки для любого архитектурно более простого решателя~--- аддитивного либо парно-аддитивного~--- на том же параметрическом семействе. Граница проверена против аналитической проекции на эталонном решении.
...
LC-PINN    & Loss-Conditional Physics-Informed Neural Network \\
...
The repository contains three top-level directories. \texttt{thesis/} holds the LaTeX source of this manuscript (builds with LuaLaTeX + biber). \texttt{code/} holds the LC-PINN training pipeline (PyTorch), with entry-point scripts under \texttt{code/scripts/} for each PDE family discussed in Section~\ref{sec:experiments}. \texttt{anova/} holds the functional-ANOVA decomposition module used in Section~\ref{subsec:anova}. Trained checkpoints are excluded from the repository but can be regenerated by running the training scripts; per-seed configuration and the resulting rel-$L^2$ numbers are recorded in the JSON output files under \texttt{code/results/} and \texttt{anova/results/}.

\subsection{Key listings}

The three listings below show the core mechanisms described in Sections~\ref{sec:method}--\ref{sec:theory}: the FiLM hypernetwork that produces the per-layer scale-and-shift parameters from the conditioning vector $\param$, the LC-PINN forward pass that applies FiLM modulation to a tanh backbone, and the L-BFGS finishing pass with the fixed-support trick described in Section~\ref{sec:method}.

\begin{lstlisting}[language=Python, caption={FiLM hypernetwork. Produces per-layer $(\gamma_\ell, \beta_\ell)$ from the conditioning vector $\param$, initialised so that $(\gamma, \beta) \approx (1, 0)$ at the start of training.}, label=lst:film]
class _FiLMHyperNet(nn.Module):
    def __init__(self, dim_lambda, layer_widths, hidden=64):
        super().__init__()
        ...
        # Identity modulation at init: gamma ~ 1, beta ~ 0.
        with torch.no_grad():
            self.trunk[-1].weight.mul_(0.01)

    def forward(self, log_lambda):
        out = self.trunk(log_lambda)
        chunks, offset = [], 0
        for h in self.layer_widths:
            gamma = 1.0 + out[..., offset:offset + h]; offset += h
            beta  = out[..., offset:offset + h];       offset += h
            chunks.append((gamma, beta))
        return chunks
\end{lstlisting}

\begin{lstlisting}[language=Python, caption={LC-PINN forward pass under FiLM conditioning. The backbone is an MLP with tanh activations; at each hidden layer the pre-activation is scale-and-shifted by $(\gamma_\ell, \beta_\ell)$ emitted by the hypernetwork.}, label=lst:lcpinn]
def forward(self, coords, log_lambda):
    if log_lambda.dim() == 1:
    ...
    if self._conditioning == "concat":
        ...
        for layer, (gamma, beta) in zip(self.linears, mods):
            ...
            h = torch.tanh(h)
        raw = self.head(h)
    return torch.sigmoid(raw) if self._output_mode == "sigmoid" else raw
\end{lstlisting}

\begin{lstlisting}[language=Python, caption={L-BFGS finishing pass with the fixed-support trick. The Monte-Carlo support of $\param$ values is frozen for \texttt{resample\_every} outer iterations so that the loss seen by L-BFGS is deterministic in $\theta$ and the strong-Wolfe line search holds; the support is then refreshed.}, label=lst:lbfgs]
def lbfgs_finish(model, batch, device, n_iter, n_k_support,
                 w_pde, w_bc, w_data, resample_every=100):
    ...
    def refresh_support():
        state["k_support"] = (torch.rand(n_k_support, device=device) * 2.0) - 1.0
    ...
    for it in range(n_iter):
        if it > 0 and it % resample_every == 0:
            refresh_support()
        opt.step(closure)
        if state["last_loss"] != state["last_loss"]:  # NaN guard
            break
\end{lstlisting}

The full implementations of these components, plus the PDE-residual computations, the Saltelli Monte-Carlo Sobol estimator, the Fourier-feature joint HDMR fit, and the training entry-point scripts for each PDE family, are at the GitHub URL above.
