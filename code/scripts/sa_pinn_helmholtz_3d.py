"""SA-PINN baseline on 3D parametric Helmholtz.

Per (seed, k_train), trains a fresh SA-PINN at that fixed wavenumber.

Usage:
    python scripts/sa_pinn_helmholtz_3d.py --seeds 0 1 \\
        --k-trains 1.0 3.0 5.0 --n-adam 5000 --n-lbfgs 2500

Output:
    results/sa_pinn_helmholtz_3d.json
    checkpoints/sa_pinn_helmholtz_3d_seed{s}_k{k:.2f}.pt
"""
...
def _per_point_residual_pde(model, coords, k_fixed):
    ...
    u = model(coords)
    lap = helm._laplacian_3d(u, coords)
    f = helm.forcing(coords[:, 0:1], coords[:, 1:2], coords[:, 2:3], k_fixed)
...
def run_one(seed, k_train, batch, device, n_adam, n_lbfgs,
            lr_theta, lr_lambda, eval_nx):
    ...
    ckpt = REPO / "checkpoints" / f"sa_pinn_helmholtz_3d_seed{seed}_k{k_train:.2f}.pt"
    torch.save({
...
def main() -> int:
    ...
    summary = {
        "method": "sa-pinn", "equation": "helmholtz_3d",
    ...
    out = REPO / "results" / "sa_pinn_helmholtz_3d.json"
