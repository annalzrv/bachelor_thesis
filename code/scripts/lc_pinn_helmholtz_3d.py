"""LC-PINN on 3D parametric Helmholtz: amortise across the wavenumber family.

Network input is (x, y, z, k_norm) with k_norm ∈ [-1, 1] and dim_lambda=1.
Mirrors the 2D script; same FiLM/L-BFGS sweep knobs.

Usage:
    python scripts/lc_pinn_helmholtz_3d.py --seeds 0 1 2 3 --n-epochs 25000 \
        --conditioning film --hidden-width 64 --n-lbfgs 1500 --tag film_lbfgs_w64

Output:
    results/lc_pinn_helmholtz_3d{_tag}.json
    checkpoints/lc_pinn_helmholtz_3d_seed{s}{_tag}.pt
"""
...
def train_lc_helmholtz_3d(
    model, batch, device, n_epochs, lr, n_k_per_step,
...
def lbfgs_finish(
    model, batch, device, n_iter, n_k_support,
    w_pde, w_bc, w_data, log_every=50, resample_every=100,
):
    """L-BFGS with frozen k-support refreshed every resample_every iters."""
...
def evaluate_at_K_k_values(model, device, K, nx=24):
    k_grid = np.linspace(helm.K_MIN, helm.K_MAX, K, dtype=np.float32)
    errs = [helm.evaluate_at_k(model, float(k), device, nx=nx, is_lc=True) for k in k_grid]
    return list(map(float, k_grid)), list(map(float, errs))


def run_one_seed(seed, batch, device, n_epochs, lr, n_k_per_step, K_eval,
                 w_pde, w_bc, w_data, conditioning, hidden_dims,
                 n_lbfgs, tag, eval_nx=24):
    ...
    t0 = time.time()
    print(f"[seed {seed}] LC-PINN 3D Helmholtz: cond={conditioning} "
    ...
    if n_lbfgs > 0:
        ...
        pre_lbfgs_err = float(np.mean(evaluate_at_K_k_values(model, device, K=K_eval, nx=eval_nx)[1]))
        ...
        post_err = float(np.mean(evaluate_at_K_k_values(model, device, K=K_eval, nx=eval_nx)[1]))
    ...
    k_vals, errs_K = evaluate_at_K_k_values(model, device, K=K_eval, nx=eval_nx)
    mean_rel = float(np.mean(errs_K))
    std_rel = float(np.std(errs_K))

    k_centre = 0.5 * (helm.K_MIN + helm.K_MAX)
    err_centre = helm.evaluate_at_k(model, k_centre, device, nx=eval_nx, is_lc=True)

    suffix = f"_{tag}" if tag else ""
    ckpt = REPO / "checkpoints" / f"lc_pinn_helmholtz_3d_seed{seed}{suffix}.pt"
    torch.save({
        ...
        "n_k_per_step": n_k_per_step, "K_eval": K_eval, "eval_nx": eval_nx,
...
def main() -> int:
    ...
    p.add_argument("--K-eval", type=int, default=10)
    p.add_argument("--eval-nx", type=int, default=24,
                   help="grid resolution per axis for rel-L² evaluation")
    ...
    print(f"Config: seeds={args.seeds} n_epochs={args.n_epochs} K_eval={args.K_eval} "
          ...
          f"tag={args.tag!r}\n", flush=True)
    print(f"3D Helmholtz family: k ∈ [{helm.K_MIN}, {helm.K_MAX}], domain "
          f"[{helm.X_MIN},{helm.X_MAX}]³", flush=True)
    ...
    for s in args.seeds:
        runs.append(run_one_seed(
            ...
            n_lbfgs=args.n_lbfgs, tag=args.tag, eval_nx=args.eval_nx,
        ))

    means = [r["mean_rel_l2"] for r in runs]
    elapsed = [r["elapsed_sec"] for r in runs]
    summary = {
        "method": "lc-pinn", "equation": "helmholtz_3d",
    ...
    out = REPO / "results" / f"lc_pinn_helmholtz_3d{suffix}.json"
