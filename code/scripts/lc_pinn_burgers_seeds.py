"""LC-PINN Burgers reference run, multiple seeds (matched bookkeeping).

Trains one LC-PINN per seed with uniform λ ∈ U(0,1)^4 sampling, identical
backbone to SA-PINN and ReLoBRaLo baselines so the head-to-head comparison
in `notebooks/09_baseline_comparison.ipynb` is on equal footing.

After training, evaluates the network at K equally spaced uniform random
λ-vectors (default K = 25) so we can also report a "mean over inference λ"
rel-L2 for the amortised-cost analysis.
...
Usage:
    python scripts/lc_pinn_burgers_seeds.py --seeds 0 1 2 3 --n-epochs 50000

Output:
    results/lc_pinn_burgers_{tag}.json
    checkpoints/lc_pinn_burgers_{tag}_seed{s}.pt
"""
...
def evaluate_at_K_lambdas(model, ref, device, K: int, eval_seed: int = 999):
    """Mean rel-L2 across K random λ ~ U(0,1)^4 at inference."""
    rng = np.random.default_rng(eval_seed)
    errs = []
    for _ in range(K):
        lam = rng.uniform(0.0, 1.0, size=4).astype(np.float32)
        log_lam = torch.tensor(np.log(np.clip(lam, 1e-8, None)), device=device)
        e = burg.evaluate(model, log_lam, ref, device)
        errs.append(float(np.mean(list(e.values()))))
    return errs


def run_one_seed(seed: int, batch, ref, device, n_epochs: int,
                 lr: float, n_lambda_samples: int, K_eval: int,
                 tag: str = "seeds", sampler_mode: str = "uniform",
    ...
    sampler = LambdaSampler(dim=burg.DIM_LAMBDA, mode=sampler_mode, device=device)

    t0 = time.time()
    print(f"[seed {seed}] LC-PINN Burgers: cond={conditioning} hidden={hidden_dims} "
    ...
    err_centre = burg.evaluate(model, lam_centre, ref, device)
    mean_centre = float(np.mean(list(err_centre.values())))

    ckpt = REPO / "checkpoints" / f"lc_pinn_burgers_{tag}_seed{seed}.pt"
    torch.save({
...
def main() -> int:
    ...
    p.add_argument("--K-eval", type=int, default=200)
    p.add_argument("--tag", type=str, default="seeds",
                   help="suffix for output filenames, e.g. 'seeds' or 'long'")
    p.add_argument("--sampler-mode", type=str, default="uniform",
                   choices=["uniform", "logspace", "simplex", "uniform_normalized"],
                   help="LambdaSampler prior mode (sampling distribution ablation)")
    ...
    print("Building Burgers reference & training batch…", flush=True)
    ...
    for s in args.seeds:
        runs.append(run_one_seed(
            ...
            args.n_lambda_samples, args.K_eval,
            tag=args.tag, sampler_mode=args.sampler_mode,
    ...
    out = REPO / "results" / f"lc_pinn_burgers_{args.tag}.json"
