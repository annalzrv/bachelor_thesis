"""LC-PINN on parametric Helmholtz: amortise across the wavenumber family.

LC-PINN here conditions the network on the *physical parameter* k (not loss
weights, as in Burgers). Network input is (x, k_norm) with k_norm ∈ [-1, 1]
and dim_lambda = 1. Custom training loop samples k uniformly per step rather
than going through `LambdaSampler` (which is built for loss-weight curricula).

After training, evaluates rel-L² at K_eval evenly-spaced k values across the
family for the amortised-cost analysis.
...
Usage:
    python scripts/lc_pinn_helmholtz.py --seeds 0 1 2 3 --n-epochs 50000

Output:
    results/lc_pinn_helmholtz{_tag}.json
    checkpoints/lc_pinn_helmholtz_seed{s}{_tag}.pt
"""
...
def train_lc_helmholtz(
    ...
    """Adam + cosine LR. Each step: average loss over n_k_per_step random k."""
    ...
    for step in range(n_epochs):
        ...
        for _ in range(n_k_per_step):
            k_norm = (torch.rand(1, device=device) * 2.0) - 1.0       # ∈ [-1, 1]
...
def evaluate_at_K_k_values(
    model, device, K: int, eval_seed: int = 999,
) -> tuple[list[float], list[float]]:
    """Mean rel-L² across K evenly-spaced k ∈ [K_MIN, K_MAX]."""
...
def run_one_seed(
    ...
    t0 = time.time()
    print(f"[seed {seed}] LC-PINN Helmholtz: cond={conditioning} "
    ...
    ckpt = REPO / "checkpoints" / f"lc_pinn_helmholtz_seed{seed}{suffix}.pt"
    torch.save({
    ...
    print(f"[seed {seed}] done in {elapsed/60:.1f} min  "
          f"rel-L2 mean over k-grid (K={K_eval}): {mean_rel:.4e} ± {std_rel:.4e}",
          flush=True)
    return {
        "seed": seed,
...
def main() -> int:
    ...
    p.add_argument("--w-bc", type=float, default=100.0,
                   help="BC must be hit exactly; up-weight by default.")
    ...
    summary = {
        "method": "lc-pinn", "equation": "helmholtz",
    ...
    out = REPO / "results" / f"lc_pinn_helmholtz{suffix}.json"
