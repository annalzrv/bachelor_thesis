"""ReLoBRaLo baseline on parametric Helmholtz.

Per (seed, k_train), trains a ReLoBRaLo-balanced fixed-weight PINN on the
Helmholtz problem at that wavenumber. Three-term loss vector
(pde, bc, data) — no IC for elliptic problems.

Usage:
    python scripts/relobralo_helmholtz.py --seeds 0 1 2 3 \\
        --k-trains 1.0 3.25 5.5 7.75 10.0 --n-epochs 50000

Output:
    results/relobralo_helmholtz.json
    checkpoints/relobralo_helmholtz_seed{s}_k{k:.2f}.pt
"""
...
class ReLoBRaLoBalancer:
    """Same balancer as relobralo_burgers.py — kept inline so this script is
    self-contained. See that file for the full reference."""
...
def run_one(seed: int, k_train: float, batch, device, n_epochs: int, lr: float,
    ...
    ckpt = REPO / "checkpoints" / f"relobralo_helmholtz_seed{seed}_k{k_train:.2f}.pt"
    torch.save({
...
def main() -> int:
    ...
    print(f"Helmholtz family: k ∈ [{helm.K_MIN}, {helm.K_MAX}]")
    ...
    grid_means_per_seed: dict[int, list[float]] = {}
    for r in runs:
    ...
    summary = {
        "method": "relobralo", "equation": "helmholtz",
    ...
    out = REPO / "results" / "relobralo_helmholtz.json"
