"""SA-PINN baseline on 2D parametric Helmholtz.

Per (seed, k_train), trains a fresh SA-PINN on the 2D Helmholtz problem at
that fixed wavenumber.

Usage:
    python scripts/sa_pinn_helmholtz_2d.py --seeds 0 1 \\
        --k-trains 1.0 5.5 10.0 --n-adam 5000 --n-lbfgs 2500

Output:
    results/sa_pinn_helmholtz_2d.json
    checkpoints/sa_pinn_helmholtz_2d_seed{s}_k{k:.2f}.pt
"""
...
def _per_point_residual_pde(model, coords, k_fixed):
    ...
    u = model(coords)
    lap = helm._laplacian_2d(u, coords)
    f = helm.forcing(coords[:, 0:1], coords[:, 1:2], k_fixed)
    return lap + (k_fixed ** 2) * u - f
...
def run_one(seed: int, k_train: float, batch, device, n_adam: int, n_lbfgs: int,
    ...
    ckpt = REPO / "checkpoints" / f"sa_pinn_helmholtz_2d_seed{seed}_k{k_train:.2f}.pt"
    torch.save({
...
def main() -> int:
    ...
    summary = {
        "method": "sa-pinn", "equation": "helmholtz_2d",
    ...
    out = REPO / "results" / "sa_pinn_helmholtz_2d.json"
