"""PI-DeepONet baseline on 1D parametric Helmholtz.

Trains a Physics-Informed DeepONet (branch over k_norm, trunk over x) using
the same residual loss as LC-PINN. No solver-generated paired data. Apples-
to-apples competitor: same family of solutions amortised in one training
run, just a different architecture.

Usage:
    python scripts/pi_deeponet_helmholtz.py --seeds 0 1 2 3 --n-epochs 50000

Output:
    results/pi_deeponet_helmholtz{_tag}.json
    checkpoints/pi_deeponet_helmholtz_seed{s}{_tag}.pt
"""
...
def compute_losses(model, k_norm, batch):
    """Mirror helm.compute_losses but call DeepONet API (coords, log_lambda)."""
...
def run_one_seed(seed, batch, device, n_epochs, lr, n_k_per_step, K_eval,
    ...
    t0 = time.time()
    print(f"[seed {seed}] PI-DeepONet 1D Helm: hidden={hidden_dims} p={n_basis} "
    ...
    ckpt = REPO / "checkpoints" / f"pi_deeponet_helmholtz_seed{seed}{suffix}.pt"
    torch.save({
...
def main() -> int:
    ...
    print(f"Config: seeds={args.seeds} n_epochs={args.n_epochs} K_eval={args.K_eval} "
          ...
          f"tag={args.tag!r}\n", flush=True)
    print(f"1D Helmholtz family: k ∈ [{helm.K_MIN}, {helm.K_MAX}]", flush=True)
    ...
    summary = {
        "method": "pi-deeponet", "equation": "helmholtz_1d",
    ...
    out = REPO / "results" / f"pi_deeponet_helmholtz{suffix}.json"
