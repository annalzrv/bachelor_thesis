"""ReLoBRaLo baseline on 2D parametric Helmholtz.

Per (seed, k_train), trains a ReLoBRaLo-balanced fixed-weight PINN on the
2D Helmholtz problem at that wavenumber. Three-term loss vector
(pde, bc, data) — no IC for elliptic problems.

Usage:
    python scripts/relobralo_helmholtz_2d.py --seeds 0 1 \\
        --k-trains 1.0 5.5 10.0 --n-epochs 25000

Output:
    results/relobralo_helmholtz_2d.json
    checkpoints/relobralo_helmholtz_2d_seed{s}_k{k:.2f}.pt
"""
...
def run_one(seed: int, k_train: float, batch, device, n_epochs: int, lr: float,
    ...
    ckpt = REPO / "checkpoints" / f"relobralo_helmholtz_2d_seed{seed}_k{k_train:.2f}.pt"
    torch.save({
...
def main() -> int:
    ...
    print(f"2D Helmholtz family: k ∈ [{helm.K_MIN}, {helm.K_MAX}]")
    ...
    summary = {
        "method": "relobralo", "equation": "helmholtz_2d",
    ...
    out = REPO / "results" / "relobralo_helmholtz_2d.json"
